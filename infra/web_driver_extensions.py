...
class WebDriverExtensions:
    def __init__(
        ...
        Args:
            driver: WebDriver implementation which will be utilize by this instance.
        """
    ...
    def __get_enabled(self, by: tuple, timeout: int) -> WebElement:
        return WebDriverWait(self.driver, timeout, ignored_exceptions).until(ec.element_to_be_clickable(by))

    def __get_exists(self, by: tuple, timeout: int) -> WebElement:
        return WebDriverWait(self.driver, timeout, ignored_exceptions).until(ec.presence_of_element_located(by))
